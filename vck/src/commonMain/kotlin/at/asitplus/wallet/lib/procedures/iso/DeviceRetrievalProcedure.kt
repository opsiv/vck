...
import at.asitplus.wallet.lib.agent.schemeIdentifierForMatching
...
internal object DeviceRetrievalProcedure {
    ...
    fun validateSubmission(
        ...
        deviceRequest.docRequests.mapIndexed { index, docRequest ->
            ...
            require(credential.schemeIdentifierForMatching == itemsRequest.docType) {
    ...
    private fun ItemsRequest.match(
        ...
        (credential as? StoreEntry.Iso)
            ?.takeIf { it.schemeIdentifierForMatching == docType }
            ?.let {
